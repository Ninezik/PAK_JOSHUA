--trx nipos
SELECT
    DATE_part('year',np.connote__created_at)tahun,
    DATE_part('month',np.connote__created_at) bulan,
    case when customer_code is null then 'RB'
    else (case when  t_salesforce.subdit_id is null or LEN(t_salesforce.subdit_id)<2 then 'EB'
    else t_salesforce.subdit_id END)
    end as subdit_id,
...
WHERE
    ...
    and np.connote__created_at <'20260801'
